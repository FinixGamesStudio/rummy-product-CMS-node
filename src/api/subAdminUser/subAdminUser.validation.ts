...
class AdminUserValidator {
  ...
  deleteSubAdminUserValidate = () =>
    validate([
      body('subAdminUserRoleId')
        .notEmpty()
        .withMessage(ERROR_MESSAGES.COMMON.REQUIRED.replace(':attribute', 'subAdminUserRoleId'))
        .isMongoId()
        .withMessage(ERROR_MESSAGES.COMMON.INVALID.replace(':attribute', 'subAdminUserRoleId'))
    ]);
  ...
  blockUnblockAdminUserValidate = () =>
    validate([
      ...
      body('adminUserRole')
        .notEmpty()
        .withMessage(ERROR_MESSAGES.COMMON.REQUIRED.replace(':attribute', 'adminUserRole'))
        .custom(async (value) => {
          const lowercaseValue = value.toLowerCase();
          const capitilizeValue = lowercaseValue.charAt(0).toUpperCase() + lowercaseValue.slice(1);
          ...
          if (USER_CONSTANT.ROLES_ARRAY.includes(capitilizeValue)) {
            return Promise.reject(
              ERROR_MESSAGES.COMMON.INVALID.replace(':attribute', 'adminUserRole')
            );
          } else {
            return Promise.resolve();
          }
